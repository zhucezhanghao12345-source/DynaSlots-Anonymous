...
from dynaslots.model.diffusion.conv1d_components import (
    Downsample1d, Upsample1d, Conv1dBlock)
from dynaslots.model.diffusion.positional_embedding import SinusoidalPosEmb
...
class TokenCrossAttention(nn.Module):
    """Multi-head attention from action features to structured obs tokens."""

    def __init__(
            self, in_dim, cond_dim, out_dim, num_heads=4,
            qk_norm=False):
        super().__init__()
        if out_dim % num_heads != 0:
            raise ValueError(
                f"out_dim {out_dim} must be divisible by {num_heads} heads"
            )
        ...
        # A zero conditioning branch preserves the original residual U-Net at
        # initialization and lets token conditioning enter progressively.
    ...
    def forward(self, x, cond):
        query = self.query_norm(self._split_heads(self.query_proj(x)))
        key = self.key_norm(self._split_heads(self.key_proj(cond)))
        value = self._split_heads(self.value_proj(cond))
        weights = torch.matmul(query, key.transpose(-2, -1)) * self.scale
        weights = weights.softmax(dim=-1)
        attended = torch.matmul(weights, value).transpose(1, 2)
        attended = attended.reshape(x.shape[0], x.shape[1], -1)
...
class ConditionalResidualBlock1D(nn.Module):
    ...
    def __init__(self,
        ...
        self.bounded_token_film = bool(bounded_token_film)
        self.bounded_token_film_bias = bool(bounded_token_film_bias)
        ...
        elif condition_type == 'token_cross_attention_add':
            self.cond_encoder = TokenCrossAttention(
                in_channels, cond_dim, out_channels,
                qk_norm=token_qk_norm,
            )
        elif condition_type == 'token_cross_attention_film':
            cond_channels = out_channels * 2
            self.cond_encoder = TokenCrossAttention(
                in_channels, cond_dim, cond_channels,
                qk_norm=token_qk_norm,
            )
    ...
    def forward(self, x, cond=None):
        ...
        if cond is not None:      
            if self.condition_type == 'film':
                ...
                bias = embed[:, 1, ...]
                out = scale * out + bias
            elif self.condition_type == 'token_cross_attention_add':
                embed = self.cond_encoder(x.permute(0, 2, 1), cond)
                out = out + embed.permute(0, 2, 1)
            elif self.condition_type == 'token_cross_attention_film':
                embed = self.cond_encoder(x.permute(0, 2, 1), cond)
                embed = embed.permute(0, 2, 1)
                embed = embed.reshape(
                ...
                scale = embed[:, 0, ...]
                bias = embed[:, 1, ...]
                if self.bounded_token_film:
                    scale = torch.tanh(scale)
                if self.bounded_token_film_bias:
                    bias = torch.tanh(bias)
                out = (1.0 + scale) * out + bias
...
class ConditionalUnet1D(nn.Module):
        ...
        if local_cond_dim is not None:
            ...
            local_cond_encoder = nn.ModuleList([
                ...
                ConditionalResidualBlock1D(
                    ...
                    condition_type=condition_type, **condition_kwargs),
                # up encoder
                ConditionalResidualBlock1D(
                    dim_in, dim_out, cond_dim=cond_dim, 
                    kernel_size=kernel_size, n_groups=n_groups,
    ...
    def forward(self, 
        ...
        if global_cond is not None:
            if self.condition_type == 'cross_attention' or \
                    self.condition_type.startswith('token_cross_attention'):
