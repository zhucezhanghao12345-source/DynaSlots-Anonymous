...
from dynaslots.model.transformer.DiT1D import DiTFeaturePredictor
...
class VisEncoder(nn.Module): 
    ...
    def forward(self, observations: Dict) -> torch.Tensor:
        ...
        if self.pointnet_type == "pointnet" or self.pointnet_type == "pointnet_avg" or self.pointnet_type == "pointnet_soft" :
            ...
            return torch.zeros(
                (points.shape[0], self.n_output_channels),
                dtype=points.dtype,
                device=points.device,
            )

    def forward_tokens(self, observations: Dict) -> torch.Tensor:
        """Return per-point tokens for object-centric binding.

        Only the point-transformer extractor exposes semantically contextual
        point tokens. Calling this method for a pooled PointNet is an error
        rather than silently repeating a global feature for every point.
        """
        if self.pointnet_type != "pointtransformer":
            raise RuntimeError(
                "DynaSlots requires pointnet_type='pointtransformer'"
            )
        points = observations[self.point_cloud_key]
        if self.use_imagined_robot:
            imagined = observations[self.imagination_key][..., :points.shape[-1]]
            points = torch.cat([points, imagined], dim=1)
        return self.extractor.forward_tokens(points)
...
class DiTFDMDecoder(nn.Module):
    def __init__(self, 
        ...
        self.fdm_decoder = DiTFeaturePredictor(
            ...
            relation_gating=relation_gating,
    ...
    def forward(self, z, t, vis, act, centroids=None,
        ...
        pred = self.fdm_decoder(
            ...
            relation_gate_strength=relation_gate_strength,
        )
        return pred
