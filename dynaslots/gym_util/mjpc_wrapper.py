...
from dynaslots.gym_util.mujoco_point_cloud import PointCloudGenerator
...
def point_cloud_sampling(point_cloud:np.ndarray, num_points:int, method:str='fps'):
    ...
    if point_cloud.shape[0] <= num_points:
        # Do not introduce synthetic zero points.  Zero padding is interpreted
        # as geometry by PointTransformer attention and can create a large,
        # task-dependent population of spurious tokens (notably on
        # MetaWorld sweep).  Repeat real points deterministically when the
        # cropped cloud is smaller than the requested resolution.
        point_cloud_dim = point_cloud.shape[-1]
        if point_cloud.shape[0] == 0:
            return np.zeros((num_points, point_cloud_dim), dtype=point_cloud.dtype)
        pad_count = num_points - point_cloud.shape[0]
        repeat_idx = np.arange(pad_count, dtype=np.int64) % point_cloud.shape[0]
        point_cloud = np.concatenate([point_cloud, point_cloud[repeat_idx]], axis=0)
        return point_cloud
