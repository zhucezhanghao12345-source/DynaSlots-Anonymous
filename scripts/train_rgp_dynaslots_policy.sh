...
pretrain_exp="${task_name}-rgp-dynaslots-${addition_info}"
exp_name="${task_name}-rgp-dynaslots_policy-${addition_info}"
...
if (( num_gpus > 1 )); then
    ...
    if (( per_gpu_batch * num_gpus != 128 )); then
        echo "GPU count ${num_gpus} must divide global batch 128" >&2
        exit 2
    fi
fi

"${launcher[@]}" train.py --config-name=rgp_dynaslots_policy.yaml \
    task="${task_name}" \
